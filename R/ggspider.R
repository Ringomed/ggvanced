...
#' @param real Whether to use the single scale for the entire input data and define the center circle
#' as zero. Defaults to FALSE.
#' If real is set to TRUE, central_distance is set to 0 and n_labels is set to 4.
...
ggspider <- function(p_data,
  ...
  # real scale
  # circle_coords is given.

  if (!is.null(real) & real == T) {

    # in real == T condition, I want central_distance to be 0.
    central_distance = 0
    # in real == T condition, I want n_labels to be 4 by default.
    if (missing(n_labels)) {
      n_labels = 4
    }

    # exclude group column and take the maximum
    rmax = max(p_data %>% select(-group))

    # Center is zero.
    # The fourth circle from the center is always the maximum.
    step_1 = purrr::map_df(seq(1 / n_labels, 1, 1 / n_labels) * rmax, circle_coords) %>%
      ggplot2::ggplot(ggplot2::aes(x, y)) +
      # plot domain is maximum times 1.3, allowing for axis labels.
      ggplot2::geom_polygon(data=circle_coords(1.3 * rmax), alpha=1, fill=background_color, lty=2) +
      ggplot2::geom_path(ggplot2::aes(group=r), lty=2, alpha=0.5) +
      ggplot2::theme_void()
    ...
    # n_labels should be the same as the number of concentric polygons.
    text_data = p_data %>%
      dplyr::select(-group) %>%
      purrr::map_df(~ rmax * seq(1 / n_labels, 1, 1 / n_labels)) %>%
      dplyr::mutate(r=seq(1 / n_labels, 1, 1 / n_labels)) %>%
      tidyr::pivot_longer(-r, names_to="parameter", values_to="value")
    ...
    labels_data = purrr::map_df(seq(1 / n_labels, 1, 1 / n_labels), text_coords) %>%
      dplyr::bind_cols(text_data %>% dplyr::select(-r)) %>%
      mutate(x=x * rmax + 0.1 * rmax, y=y * rmax + 0.05 * rmax, r=r * rmax, value=as.character(signif(value, 3)))
    if(!is.null(reorder_axis)) {
      labels_data = labels_data %>% dplyr::mutate(value=case_when(parameter %in% reorder_axis ~ -value,
                                                                  TRUE ~ value))
    }

    # rescaled_coords is given.
    pdata_long_rescaled = p_data %>%
      tidyr::pivot_longer(-1, names_to="parameter", values_to="mean") %>%
      select(group, parameter, y=mean) %>%
      mutate(measure="mean") %>%
    ...
    rescaled_data = pdata_long_rescaled %>% filter(measure == "mean") %>%
      select(-measure) %>%
      pivot_wider(names_from="parameter", values_from="y") %>%
      # this copies the second column to the last column. ensures geom_polygon to be closed
      dplyr::mutate(copy=dplyr::pull(., 2)) %>%
      tidyr::pivot_longer(-group, names_to="parameter", values_to="value") %>%
      dplyr::group_by(group) %>%
      dplyr::mutate(coords=rescaled_coords(value + central_distance, NCOL(p_data) - 1)) %>%
      tidyr::unnest(cols=c(coords))
    ...
    step_1 +
      {if(draw_axis == T) ggplot2::geom_line(data=axis_coords(NCOL(p_data) - 1), ggplot2::aes(x, y, group=id), alpha=0.3)} +
      scale_linewidth(range=c(0, 4)) +
      ggplot2::geom_point(data=rescaled_data, ggplot2::aes(x, y, group=group, col=group), size=2, stroke=2) +
      ggplot2::geom_path(data=rescaled_data, ggplot2::aes(x, y, group=group, col=group), size=1) +
      {if(area_fill == T) ggplot2::geom_polygon(data=rescaled_data, ggplot2::aes(x, y, group=group, col=group, fill=group), size=1, alpha=fill_opacity, show.legend=F)} +
      ggplot2::geom_text(data=labels_data %>% dplyr::filter(parameter == labels_data$parameter[[1]]), ggplot2::aes(x, y, label=value), alpha=0.65,
                         family=theme_get()$text[["family"]],
                         size=ifelse(is.null(axis_label_font_size), theme_get()$text[["size"]]/2.75, axis_label_font_size),
                         fontface=ifelse(is.null(axis_label_font_face), "plain", axis_label_font_face)) +
      ggplot2::geom_text(data=text_coords(1.15 * rmax), ggplot2::aes(x, y), label=labels_data$parameter[1:(NCOL(p_data) - 1)],
                         family=theme_get()$text[["family"]],
                         size=ifelse(is.null(axis_name_font_size), theme_get()$text[["size"]]/2.75, axis_name_font_size),
                         fontface=ifelse(is.null(axis_name_font_face), "plain", axis_name_font_face)) +
      ggplot2::labs(col=legend_title) +
